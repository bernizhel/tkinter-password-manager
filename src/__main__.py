...
class App(tk.Tk):
    ...
    def _paint(self):
        ...
        self._entries['canvas'] = \
            tk.Canvas(self._entries['frame'],
                      width=self.WINDOW_WIDTH, height=self.WINDOW_HEIGHT)
        self._entries['canvas'].grid(row=0, column=0, padx=self.PADDING, pady=self.PADDING)

        self._entries['scrollbar'] = \
            tk.Scrollbar(self._entries['frame'], orient='vertical', command=self._entries['canvas'].yview)
        ...
        self._commands['save']['button'] = \
            tk.Button(self._commands['frame'], text='Save', command=self._ask_save)
        self._commands['save']['button'].grid(row=0, column=0, padx=self.PADDING)

        self._commands['add']['button'] = \
            tk.Button(self._commands['frame'], text='Add new', command=self._ask_add_entry)
    ...
    def _paint_entries(self):
        ...
        for entry_id, entry_data in entries.items():
            ...
            self._entries['entries'][row_index]['number']['label'] = \
                tk.Label(self._entries['scrollable'], text=row_index + 1, width=self.NUMBER_WIDTH, justify='left')
            self._entries['entries'][row_index]['number']['label'].grid(row=row_index, column=0)
    ...
    def _app_exit(self):
        answer = messagebox.askyesnocancel('Quit', 'Quitting. Save the changes?')
...
class ModalWindow(tk.Toplevel):
    ...
    def _confirm_command(self):
        ...
        try:
            ...
            if not self._confirm['will_destroy_on_error']:
                return
            self.destory()
        else:
            self._repaint()
...
class FernetEncryptor:
    ...
    def encrypt(self, data='', password='', iterations=ITERATIONS):
        ...
        encrypted_data = salt
        encrypted_data += iterations.to_bytes(4, 'big')
        encrypted_data += b64decode(fernet.Fernet(key).encrypt(data.encode()))
    ...
    def decrypt(self, token=b'', password=''):
        ...
        decrypted_data = fernet.Fernet(key).decrypt(token).decode()
...
class EntriesStorage:
    ...
    def _database_path(self):
        return os.path.join(os.path.dirname(__file__), self._database_file_name)
    ...
    def save(self, password=''):
        ...
        with open(self._database_path, 'wb') as database_file:
